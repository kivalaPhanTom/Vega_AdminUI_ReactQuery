...
import {checkIsChangedData} from '../validator'

function ModalEdit(props) {
    const dispatch = useDispatch();
    const isOpenConfirmEdit = useSelector((state)=> state.employeesSlice.isOpenConfirmEdit)
    const data = useSelector((state)=> state.employeesSlice.dataEdit)
    const cacheDataEdit = useSelector((state)=> state.employeesSlice.cacheDataEdit)
    ...
    const handleOk = async() => {
        ...
        if(employeeID.trim() !== "" && user_name.trim() !== "" && user_password.trim() !== "" && user_email.trim() !== "" && birthDate !== null
           ...
           workingAddress.trim() !== '' && fileList.length > 0){
            ...
            const isChangedData = checkIsChangedData(cacheDataEdit, dataSubmit)
